...
- **Feb 2026**: Added **Reserve Price** support to Strategy Launcher (auctions). New `Reserve %` input (0–100, default 0 = no reserve) in the Pricing settings step. Backend `/api/schedule/launch-strategy` now accepts `auction_reserve_pct` and per-card stores `reserve_price = price × pct/100`. Already supported in manual Add Cards flow + `/api/schedule/add-bulk` + worker XML payload via `<ReservePrice currencyID="USD">`. Also cleaned 14 lines of corrupted leftover content at end of `StrategyLauncher.jsx`.
- **Feb 2026**: Fixed Schedule worker failures — (1) basketball/football categories rejected ConditionID=4000; now hardcodes `PrimaryCategory=261328` with `CategoryMappingAllowed=true` (matches manual listing flow). (2) Image upload helper now raises RuntimeError with parsed eBay error instead of silent None return. (3) Added rich AI description to scheduled listings using `generate_listing_description` + `generate_hype_content` + `build_hype_description` (previously only used title).
- **Feb 2026**: Fixed `.gitignore` to allow `frontend/build/` to be tracked — was blocking production deploys via `bash fix.sh`.
- **Feb 2026**: Fixed Schedule UI grouping to use LOCAL date instead of `toISOString()` UTC — posts scheduled Monday 7pm CT no longer appear on Tuesday.
- **Feb 2026**: Added scroll position persistence in Inventory and Listings modules — returning from edit/detail view restores scroll to where user was.
- **Feb 2026**: Added `POST /api/schedule/bulk/retry-failed` endpoint + "Retry All Failed" button in Schedule History.
- **Feb 2026**: **NEW FEATURE — Pull Game** (Chase Cards section). Interactive live-pull game where admins build a game with chasers (mini/mid/blue) + low-end pool + 4 Mega Box cards; buyers purchase numbered pulls via Stripe checkout (guest or logged-in), reveal animates a card result. Blue Chase unlocks a Mega Box selection that resets the game. Admin panel for game lifecycle (create/pause/end/ship). Public guest pages at `/pull-game/:gameId` and `/pull-game/:gameId/reveal`. Sidebar "Chase Packs" renamed to "Chase Cards" with 2 tabs (Packs + Pull Game). 12 new backend endpoints in `/app/backend/routers/pull_game.py`. Frontend: 4 new files (ChaseCardsModule, PullGameModule, PullGamePublicPage, PullGameRevealPage). Testing agent 23/23 backend pass + frontend guest flow validated.
- **Feb 2026**: **Pull Game UX overhaul** — (1) 3-column "Recipe Box" layout (Blue Box left, Recipe Box center with stacked file-tab pulls, Orange Box right; Red Chases below). (2) Dynamic per-pull tier pricing — price uses `pulls_sold + idx`, so subsequent cart pulls escalate correctly across tiers. (3) Exact mechanics: 6 Red Chases + 1 Orange trigger (4 Orange Box options) + 1 Blue trigger (4 Blue Box options). (4) Game auto-ends when BOTH Orange and Blue triggers picked. (5) **Multi-Select Cart**: buyers can add multiple pull spots into a Set-based cart, checkout in ONE Stripe transaction with combined per-tier total. CartFloat sticky bar with chips, total, and clear/checkout actions. (6) **Sequential Reveals**: post-payment state machine reveals each spot one-by-one with animations; trigger spots open BoxSelect (4 boxes) before continuing; final summary grid lists all revealed cards. Backend `buy-pull` accepts `pull_numbers` array (legacy `pull_number` still supported); checkout/status returns BOTH `spots` array + legacy `spot`. box-pick accepts optional `trigger_type` to disambiguate when cart contains both triggers. Atomic reservation: any failed spot rolls back ALL reservations (including new Stripe API failure rollback). Testing agent: 8/8 backend pytest + frontend live cart + reveal flow verified.
