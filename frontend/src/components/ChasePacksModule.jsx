        ...
        <div className="bg-[#111] border border-white/[0.06] rounded-xl overflow-hidden" data-testid="buyers-panel">
          <div className="px-4 py-2.5 border-b border-white/[0.06] flex items-center justify-between">
            <h3 className="text-sm font-bold text-white flex items-center gap-2">
              <Users className="w-4 h-4 text-[#3b82f6]" /> Buyers ({assignedCards.length})
            </h3>
            <span className="text-xs text-gray-500">{assignedCards.filter(c => c.revealed).length} revealed</span>
          </div>
          <div className="divide-y divide-white/[0.04]">
            {assignedCards.map((card, idx) => (
              <div key={idx} className="flex items-center gap-4 px-4 py-2.5 hover:bg-white/[0.02] transition-colors" data-testid={`buyer-row-${idx}`}>
                {/* # */}
                <span className="text-xs text-gray-600 font-mono w-5 shrink-0">{idx + 1}</span>
                {/* Thumbnail */}
                <div className="w-8 h-11 rounded overflow-hidden border border-white/[0.06] shrink-0">
                  {card.image ? (
                ...
                {/* Buyer + Card info */}
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-[#3b82f6] truncate">{card.assigned_to}</p>
                ...
                <div className="shrink-0">
                  {card.revealed ? (
                    ...
                    <span className="inline-flex items-center gap-1 text-xs font-bold text-amber-400 bg-amber-500/10 px-2.5 py-1 rounded-lg">
                ...
                <div className="shrink-0 flex items-center gap-1.5">
                  <code className="text-sm font-mono font-bold text-white bg-[#0a0a0a] border border-white/[0.08] px-3 py-1.5 rounded-lg tracking-wider">
                    {card.claim_code}
                  </code>
                  <button onClick={() => copyCode(card.claim_code)}
                    className="p-2 rounded-lg bg-white/[0.04] hover:bg-[#f59e0b]/20 hover:text-[#f59e0b] text-gray-400 transition-all" data-testid={`copy-code-${idx}`}>
                ...
                <div className="shrink-0 flex items-center gap-1.5">
                  <button onClick={() => regenerateCode(card.card_id)} disabled={!!actionLoading}
                    className="px-3 py-1.5 rounded-lg bg-purple-500/10 border border-purple-500/20 text-purple-400 text-[10px] font-bold hover:bg-purple-500/20 disabled:opacity-40 transition-all flex items-center gap-1"
                    data-testid={`regen-code-${idx}`}>
                    {actionLoading === `regen-${card.card_id}` ? <RefreshCw className="w-3 h-3 animate-spin" /> : <KeyRound className="w-3 h-3" />} New Code
                  </button>
                  {isEditable && (
                    <button onClick={() => setConfirm({ title: 'Unassign Buyer', message: `Remove ${card.assigned_to} from this spot? The card will become available again.`, danger: true, onConfirm: () => unassignBuyer(card.card_id), onCancel: () => setConfirm(null) })}
                      disabled={!!actionLoading}
                      className="px-3 py-1.5 rounded-lg bg-red-500/5 border border-red-500/15 text-red-400 text-[10px] font-bold hover:bg-red-500/10 disabled:opacity-40 transition-all flex items-center gap-1"
