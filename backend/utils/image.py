...
def scanner_auto_process(image_base64: str, is_back: bool = False) -> str:
    """Auto-crop scanner image: fixed pixel cut from each edge of the IMAGE (not the card).
    Calibrated for the current scanner output (~727x1022).
    User-marked crop rectangle: TOP=150, SIDE=130, BOT=72.
    """
    try:
        import cv2
        import numpy as np
